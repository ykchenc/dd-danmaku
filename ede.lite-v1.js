...
(async function () {
    ...
    const buildProfile = globalThis.__EDE_BUILD_PROFILE__ || 'full';
    const isLiteV1 = buildProfile === 'lite-v1';
    const featureFlags = {
        enableHashMatch: !isLiteV1, // O01 + O02
        enableAppLogAspect: !isLiteV1, // O03
        enableDanmakuInfoTab: !isLiteV1, // O12
        enableBangumiCharacters: !isLiteV1, // O13
        enableOsdLineChart: !isLiteV1, // O16
        enableMergeSimilar: !isLiteV1, // O06
        enableFontCustomization: !isLiteV1, // O08
    ...
    ].filter(tab => featureFlags.enableDanmakuInfoTab || tab.id !== currentDanmakuInfoContainerId);
    // 弹幕类型过滤
    ...
    function danmakuMergeSimilar(comments, threshold = 50, timeWindow = 15) {
        if (!featureFlags.enableMergeSimilar) { return comments; }
    ...
    function buildDanmakuSetting(containerId) {
        ...
        if (featureFlags.enableFontCustomization) {
            ...
            const fontSettingCollapse = container.querySelector('div[is="emby-collapse"][title="弹幕字体样式"]');
    ...
    function buildProSetting(containerId) {
        ...
        if (!featureFlags.enableMergeSimilar) {
            [eleIds.danmakuFilterProDiv, eleIds.mergeSimilarPercentDiv, eleIds.mergeSimilarTimeDiv]
                .forEach(id => {
                    const ele = getById(id, container);
        ...
        if (!featureFlags.enableOsdLineChart) {
            const osdLineChartDiv = getById(eleIds.osdLineChartDiv, container);
            if (osdLineChartDiv && osdLineChartDiv.parentElement) {
                osdLineChartDiv.parentElement.style.display = 'none';
            }
            const osdLineChartTimeDiv = getById(eleIds.osdLineChartTimeDiv, container);
    ...
    function buildAbout(containerId) {
        ...
        if (featureFlags.enableConsoleLogViewer) {
            ...
            const consoleLogInfoEle = getById(eleIds.consoleLogInfo, container);
            if (consoleLogInfoEle) {
                consoleLogInfoEle.remove();
            }
            container.querySelectorAll(`#${eleIds.consoleLogCtrl}`).forEach(ele => ele.remove());
        }
